<template>
  <div>
    <h6>Main</h6>
    <nuxt-link to="/" :class="{'sl-active': getActiveLink('/')}"><font-awesome-icon :icon="['fas', 'fire']" fixed-width />&ensp;Dank Memes</nuxt-link>
    <nuxt-link v-if="$auth.loggedIn" to="/feed" :class="{'sl-active': getActiveLink('/feed')}"><font-awesome-icon :icon="['fas', 'carrot']" fixed-width />&ensp;Feed</nuxt-link>
    ...
    <nuxt-link to="/all" :class="{'sl-active': getActiveLink('/all')}"><font-awesome-icon :icon="['fas', 'bars']" fixed-width />&ensp;Browse all</nuxt-link>
    <template v-if="getModerating.length">
      <hr>
      <h6>Pages</h6>
      <nuxt-link v-for="m in getModerating" :key="m.name" :to="'/page/'+m.name">
    ...
    <template v-if="getSubscriptions.length">
      <hr>
      <h6>Subscriptions</h6>
      <nuxt-link v-for="s in getSubscriptions" :key="s.name" :to="'/page/'+s.name">
