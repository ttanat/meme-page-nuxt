...
<div>
  <div class="container-fluid content-section" style="width: 98%;margin-bottom: 5px;">
    <div class="row">
      <div class="col" style="{padding: meme.caption ? '' : '15px'}">
        ...
        <a :href="'/user/' + meme.username" style="color: gainsboro;"><img v-if="meme.dp_url" class="rounded-circle" :src="'http://127.0.0.1:8000'+meme.dp_url" height="24" width="24"><font-awesome-icon v-else :icon="['fas', 'user-circle']" style="font-size: 20px;" />&ensp;{{ meme.username }}</a>
        <span v-if="meme.pname" class="text-muted" style="font-size: 14px;">&ensp;<font-awesome-icon :icon="['fas', 'caret-right']" />&ensp;<a class="text-muted" :href="'http://127.0.0.1:8000/' + meme.pname">{{ meme.pdname || meme.pname }}</a></span>
      </div>
      <div class="col d-none d-lg-inline" id="rand-col" style="padding: 20px;">
        <a class="btn btn-sm btn-success float-right" href="/random">Random <font-awesome-icon :icon="['far', 'arrow-alt-circle-right']" /></a>
      </div>
    </div>
  </div>
  <img v-if="['image/jpeg', 'image/png'].includes(meme.content_type)" class="content w-100" :src="'http://127.0.0.1:8000'+meme.url" onclick="overlayOn()" @contextmenu.prevent style="cursor: zoom-in;">
  <video v-else-if="isGif" ref="vidMeme" @contextmenu.prevent class="content w-100" style="max-height: 85vh;" loop autoplay muted playsinline>
    <source :src="'http://127.0.0.1:8000'+meme.url">
  </video>
      ...
      <td>
        <div class="dropdown float-right" id="item-page-dropdown">
          ...
          <div class="dropdown-menu bg-dark">
            <div class="dropdown-item m-dropdown"><a :href="'/img?m=' + meme.uuid" target="_blank"><font-awesome-icon :icon="['fas', 'download']" /> Download</a></div>
  ...
  <span v-if="tags.length" class="text-muted content-section">
    <button v-for="tag in tags" :key="tag" @click="openLink(tag)" class="tag btn btn-outline-primary">{{ tag }}</button>&ensp;
  </span>
</div>
</template>
