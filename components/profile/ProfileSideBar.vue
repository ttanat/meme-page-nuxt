<template>
  <div class="col-md-4 col-xl-3" id="profile-col">
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div>
            ...
            {% else %}
                <span id="follow-btn"><follow-button :following="true" :is-profile="true"></follow-button></span>
            {% endif %}
        </div>
      </div>
    </div>
    ...
    <div class="mt-2">
      <div v-if="$route.path.startsWith('/profile')" id="vue-bio"><bio-desc bio-or-desc="user.bio" add-text="bio"></bio-desc></div>
      <span v-else style="font-size: 14px;white-space: pre-wrap;">user_displayed.bio</span>
    </div>
    <hr class="mb-2" style="background-color: grey;">

    <div v-if="$route.path !== '/profile/settings'" id="pstats"><profile-stats></profile-stats></div>
